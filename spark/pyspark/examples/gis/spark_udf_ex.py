...
def run_st_point(spark):
    ...
    rs = spark.sql("select ST_Point(x, y) from points").collect()
    for i in range(10):
...
def run_st_intersection(spark):
    ...
    rs = spark.sql("select ST_Intersection(left, right) from intersection").collect()
...
def run_st_isvalid(spark):
    ...
    rs = spark.sql("select ST_IsValid(geos) from valid").collect()
    assert rs[0][0]
    assert rs[1][0]
...
def run_st_equals(spark):
    ...
    rs = spark.sql("select ST_Equals(left, right) from equals").collect()
    assert rs[0][0]
    assert rs[1][0]
...
def run_st_touches(spark):
    ...
    rs = spark.sql("select ST_Touches(left, right) from touches").collect()
    assert not rs[0][0]
    assert rs[1][0]
...
def run_st_overlaps(spark):
    ...
    rs = spark.sql("select ST_Overlaps(left, right) from overlaps").collect()
    assert rs[0][0]
    assert not rs[1][0]
...
def run_st_crosses(spark):
    ...
    rs = spark.sql("select ST_Crosses(left, right) from crosses").collect()
    assert rs[0][0]
    assert not rs[1][0]
...
def run_st_issimple(spark):
    ...
    rs = spark.sql("select ST_IsSimple(geos) from simple").collect()
    assert not rs[0][0]
    assert not rs[1][0]
...
def run_st_geometry_type(spark):
    ...
    rs = spark.sql("select ST_GeometryType(geos) from geometry_type").collect()
...
def run_st_make_valid(spark):
    ...
    rs = spark.sql("select ST_MakeValid(geos) from make_valid").collect()
...
def run_st_simplify_preserve_topology(spark):
    ...
    rs = spark.sql("select ST_SimplifyPreserveTopology(geos, 10) from simplify_preserve_topology").collect()
...
def run_st_contains(spark):
    ...
    rs = spark.sql("select ST_Contains(left, right) from contains").collect()
    assert rs[0][0]
    assert not rs[1][0]
...
def run_st_intersects(spark):
    ...
    rs = spark.sql("select ST_Intersects(left, right) from intersects").collect()
    assert rs[0][0]
    assert not rs[1][0]
...
def run_st_within(spark):
    ...
    rs = spark.sql("select ST_Within(left, right) from within").collect()
    assert rs[0][0]
    assert not rs[1][0]
...
def run_st_distance(spark):
    ...
    rs = spark.sql("select ST_Distance(left, right) from distance").collect()
    assert rs[0][0] == 3
...
def run_st_area(spark):
    ...
    rs = spark.sql("select ST_Area(geos) from area").collect()
    assert rs[0][0] == 200
    assert rs[1][0] == 600
...
def run_st_centroid(spark):
    ...
    rs = spark.sql("select ST_Centroid(geos) from centroid").collect()
...
def run_st_length(spark):
    ...
    rs = spark.sql("select ST_Length(geos) from length").collect()
...
def run_st_hausdorffdistance(spark):
    ...
    rs = spark.sql("select ST_HausdorffDistance(geo1,geo2) from hausdorff").collect()
    assert rs[0][0] == 1
    assert rs[1][0] == 1
...
def run_st_convexhull(spark):
    ...
    rs = spark.sql("select ST_convexhull(geos) from convexhull").collect()
...
def run_st_npoints(spark):
    ...
    rs = spark.sql("select ST_NPoints(geos) from npoints").collect()
    assert rs[0][0] == 4
    assert rs[1][0] == 4
...
def run_st_envelope(spark):
    ...
    rs = spark.sql("select ST_Envelope(geos) from envelope").collect()
...
def run_st_buffer(spark):
    ...
    rs = spark.sql("select ST_Buffer(geos, 1.2) from buffer").collect()
...
def run_st_union_aggr(spark):
    ...
    rs = spark.sql("select ST_Union_Aggr(geos) from union_aggr1").collect()
    ...
    rs = spark.sql("select ST_Union_Aggr(geos) from union_aggr2").collect()
    ...
    rs = spark.sql("select ST_Union_Aggr(geos) from union_aggr3").collect()
    ...
    rs = spark.sql("select ST_Union_Aggr(geos) from union_aggr4").collect()
...
def run_st_envelope_aggr(spark):
    ...
    rs = spark.sql("select ST_Envelope_Aggr(geos) from envelope_aggr").collect()
...
def run_st_transform(spark):
    ...
    buffer_df = spark.createDataFrame(data=test_data, schema=['geos']).cache()
    buffer_df.createOrReplaceTempView("buffer")
    rs = spark.sql("select ST_Transform(geos, 'epsg:4326', 'epsg:3857') from buffer").collect()
...
def run_st_curvetoline(spark):
    ...
    buffer_df = spark.createDataFrame(data=test_data, schema=['geos']).cache()
    buffer_df.createOrReplaceTempView("buffer")
    rs = spark.sql("select ST_CurveToLine(geos) from buffer").collect()
