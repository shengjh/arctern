...
def run_st_point(spark):
    ...
    rs = spark.sql("select ST_Point(x, y) from points").cache().show()
...
def run_st_geomfromgeojson(spark):
    ...
    rs = spark.sql("select ST_GeomFromGeoJSON(json) from json").cache().show()
...
def run_st_pointfromtext(spark):
    ...
    rs = spark.sql("select ST_PointFromText(data) from data").cache().count()
...
def run_st_polygonfromtext(spark):
    ...
    data_df = spark.createDataFrame(data=test_data, schema=["data"]).cache()
    data_df.createOrReplaceTempView("data")
    rs = spark.sql("select ST_PolygonFromText(data) from data").cache().count()
...
def run_st_astext(spark):
    ...
    data_df = spark.createDataFrame(data=test_data, schema=["data"]).cache()
    data_df.createOrReplaceTempView("data")
    rs = spark.sql("select ST_AsText(ST_PolygonFromText(data)) from data").cache().count()
...
def run_st_precision_reduce(spark):
    ...
    rs = spark.sql("select ST_PrecisionReduce(geos, 4) from precision_reduce").cache().count()
...
def run_st_linestringfromtext(spark):
    ...
    data_df = spark.createDataFrame(data=test_data, schema=["data"]).cache()
    data_df.createOrReplaceTempView("data")
    rs = spark.sql("select ST_LineStringFromText(data) from data").cache().count()
...
def run_st_geomfromwkt(spark):
    ...
    data_df = spark.createDataFrame(data=test_data, schema=["data"]).cache()
    data_df.createOrReplaceTempView("data")
    rs = spark.sql("select ST_GeomFromWKT(data) from data").cache().count()
...
def run_st_geomfromtext(spark):
    ...
    data_df = spark.createDataFrame(data=test_data, schema=["data"]).cache()
    data_df.createOrReplaceTempView("data")
    rs = spark.sql("select ST_GeomFromText(data) from data").cache().count()
...
def run_st_intersection(spark):
    ...
    rs = spark.sql("select ST_Intersection(left, right) from intersection").cache().count()
...
def run_st_isvalid(spark):
    ...
    rs = spark.sql("select ST_IsValid(geos) from valid").cache().count()
...
def run_st_equals(spark):
    ...
    rs = spark.sql("select ST_Equals(left, right) from equals").cache().count()
...
def run_st_touches(spark):
    ...
    rs = spark.sql("select ST_Touches(left, right) from touches").cache().count()
...
def run_st_overlaps(spark):
    ...
    rs = spark.sql("select ST_Overlaps(left, right) from overlaps").cache().count()
...
def run_st_crosses(spark):
    ...
    rs = spark.sql("select ST_Crosses(left, right) from crosses").cache().count()
...
def run_st_issimple(spark):
    ...
    rs = spark.sql("select ST_IsSimple(geos) from simple").cache().count()
...
def run_st_geometry_type(spark):
    ...
    rs = spark.sql("select ST_GeometryType(geos) from geometry_type").cache().count()
...
def run_st_make_valid(spark):
    ...
    rs = spark.sql("select ST_MakeValid(geos) from make_valid").cache().count()
...
def run_st_simplify_preserve_topology(spark):
    ...
    rs = spark.sql("select ST_SimplifyPreserveTopology(geos, 10) from simplify_preserve_topology").cache().count()
...
def run_st_polygon_from_envelope(spark):
    ...
    rs = spark.sql("select ST_PolygonFromEnvelope(min_x, min_y, max_x, max_y) from polygon_from_envelope").cache().count()
...
def run_st_contains(spark):
    ...
    rs = spark.sql("select ST_Contains(left, right) from contains").cache().count()
    # assert rs[0][0]
    # assert not rs[1][0]
...
def run_st_intersects(spark):
    ...
    rs = spark.sql("select ST_Intersects(left, right) from intersects").cache().count()
    # assert rs[0][0]
    # assert not rs[1][0]
...
def run_st_within(spark):
    ...
    rs = spark.sql("select ST_Within(left, right) from within").cache().count()
    # assert rs[0][0]
    # assert not rs[1][0]
...
def run_st_distance(spark):
    ...
    rs = spark.sql("select ST_Distance(left, right) from distance").cache().count()
    # assert rs[0][0] == 3
...
def run_st_area(spark):
    ...
    rs = spark.sql("select ST_Area(geos) from area").cache().count()
    # assert rs[0][0] == 200
    # assert rs[1][0] == 600
...
def run_st_centroid(spark):
    ...
    rs = spark.sql("select ST_Centroid(geos) from centroid").cache().count()
...
def run_st_length(spark):
    ...
    rs = spark.sql("select ST_Length(geos) from length").cache().count()
...
def run_st_hausdorffdistance(spark):
    ...
    rs = spark.sql("select ST_HausdorffDistance(geo1,geo2) from hausdorff").cache().count()
    # assert rs[0][0] == 1
    # assert rs[1][0] == 1
...
def run_st_convexhull(spark):
    ...
    rs = spark.sql("select ST_convexhull(geos) from convexhull").cache().count()
...
def run_st_npoints(spark):
    ...
    rs = spark.sql("select ST_NPoints(geos) from npoints").cache().count()
    # assert rs[0][0] == 4
    # assert rs[1][0] == 4
...
def run_st_envelope(spark):
    ...
    rs = spark.sql("select ST_Envelope(geos) from envelope").cache().count()
...
def run_st_buffer(spark):
    ...
    rs = spark.sql("select ST_Buffer(geos, 1.2) from buffer").cache().count()
    # assert rs[0][
...
def run_st_union_aggr(spark):
    ...
    rs = spark.sql("select ST_Union_Aggr(geos) from union_aggr1").cache().count()
    ...
    # rs = spark.sql("select ST_Union_Aggr(geos) from union_aggr2").cache().count()
    ...
    # rs = spark.sql("select ST_Union_Aggr(geos) from union_aggr3").cache().count()
    ...
    # rs = spark.sql("select ST_Union_Aggr(geos) from union_aggr4").cache().count()
...
def run_st_envelope_aggr(spark):
    ...
    rs = spark.sql("select ST_Envelope_Aggr(geos) from envelope_aggr").cache().count()
...
def run_st_transform(spark):
    ...
    buffer_df = spark.createDataFrame(data=test_data, schema=['geos']).cache()
    buffer_df.createOrReplaceTempView("buffer")
    rs = spark.sql("select ST_Transform(geos, 'epsg:4326', 'epsg:3857') from buffer").cache().count()
...
def run_st_curvetoline(spark):
    ...
    buffer_df = spark.createDataFrame(data=test_data, schema=['geos']).cache()
    buffer_df.createOrReplaceTempView("buffer")
    rs = spark.sql("select ST_CurveToLine(geos) from buffer").cache().count()
